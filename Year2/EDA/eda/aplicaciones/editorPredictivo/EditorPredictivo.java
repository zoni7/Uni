...
public class EditorPredictivo extends ABB<String> {
    ...
    public ListaConPI<String> recuperarSucesores(String prefijo, int n) {       
        ...
        // Try to recover the prefix. If exists, add it to the list
        if (this.recuperar(prefijo) != null) {
            list.insertar(prefijo);
        }
        // Recover succesors till n or find one that doesn't begin with "prefijo"
        String prefijoAux = prefijo; 
        for (int i= 0; i<= n && sucesor(prefijoAux).startsWith(prefijo); i++) {
            list.insertar(sucesor(prefijoAux));
            prefijoAux = sucesor(prefijoAux);
