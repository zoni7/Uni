...
public class BuscadorDeLaBibl {
    ...
    public ListaConPI<Termino> hapax() {
        ...
        ListaConPI<Termino> list= index.claves();
        
        
        // map index
        for(list.inicio(); !list.esFin() ;list.siguiente()) {
            ListaConPI<BuscadorDeLaBibl.Posting> valor = index.recuperar(list.recuperar());
            if (valor.talla() == 1) {
                res.insertar(list.recuperar());
            }
            //Termino x = index.recuperar(list.recuperar());
        }
