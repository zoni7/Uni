...
public class GrafoNoDirigido extends GrafoDirigido {
    ...
    /** Construye un grafo No Dirigido vacio con numV vertices. 
     ...
     *  y No Ponderado; por tanto, también inserta la arista (j, i).
     *  @param i    Vertice origen
     *  @param j    Vertice destino
     */ 
    public void insertarArista(int i, int j) {
        if (!existeArista(i, j)) { 
            elArray[i].insertar(new Adyacente(j, 1.0)); 
            elArray[j].insertar(new Adyacente(i, 1.0));
            numA++; //OJO
        }
    }
    
    /** Si no esta, inserta la arista (i, j) de peso p en un grafo  
     *  No Dirigido y Ponderado; por tanto, también inserta la arista 
     *  (j, i) de peso p.
     * @param i    Vertice origen
     * @param j    Vertice destino
     * @param p    Peso de (i, j)
