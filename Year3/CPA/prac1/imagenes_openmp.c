...
int lee_ppm(char *nomfich, struct pixel ***img, int *nf, int *nc)
{
  ...
  // This loop can not be parallelized because of the i - 1
  for (i = 1; i < height; i++)
    (*img)[i] = (*img)[i - 1] + width;
           ...
           int n, int m)
{
  ...
  // Can not be parallelized bc of i - 1
  ...
  t1 = omp_get_wtime();
  for (p = 0; p < pasos; p++) {
  #pragma omp parallel for private(j, k, l, resultado, tot, v) 
  //reduction(+:tot, +:resultado)
    for (i = 0; i < n; i++) {
      for (j = 0; j < m; j++) {
        resultado.r = 0;
        resultado.g = 0;
