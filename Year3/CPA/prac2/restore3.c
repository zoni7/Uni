...
void process( int w,int h,Byte a[], int bw,int bh ) {
  int x,y, x2,y2, mx,my,min, max, d, numY2;
  ...
  for ( y = bh ; y < h ; y += bh ) {
   ...
   #pragma omp parallel private(numY2)
   {
      // Counter of Y2 Iterations of each thread
      numY2 = 0;
      ...
      for ( y2 = y ; y2 < h ; y2 += bh ) {
        ...
        if ( d > max ) { max = d; /*my = y2;*/ }
      }
      // Get only the messages from the first loop iteration of y  
      if (y == bh) { 
        printf("Thread %d: with %d iterations with distances between %d and %d \n", omp_get_thread_num(), numY2, min, max);
      } 
   } 
